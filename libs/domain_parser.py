    ...
    if check_up(url):
        try:
            response = requests.get(
                "https://" + url, headers={"User-Agent": "Google Chrome"}, timeout=1
            )
        except:
            try:
                if check_up(url):
        ...
        if (
            ...
            for port, service in ip_dict[ip]["ports"].items():
                if service == "website":
                    ...
                    try:
                        result = webtech.WebTech(options={"json": True}).start_from_url(
                            f"http://{ip}:{port}"
                        )
                    except:
                        try:
                            result = webtech.WebTech(
                                options={"json": True}
                            ).start_from_url(f"https://{ip}:{port}")
                        except:
                            result = {}
                    if result != {}:
        ...
        if mode == "WR" or mode == "A":
            for subdomain in ip_dict[ip]["subdomains"]["subdomain_with_redirect"]:
                ...
                try:
                    ...
                    try:
                        result = webtech.WebTech().start_from_url(
                            f"https://{subdomain}"
                        )
                    except:
                        result = {}
