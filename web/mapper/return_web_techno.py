...
def main(result: result) -> dict:
    """
    This function takes in a result object and returns a dictionary containing information about the technologies used by each IP address and port.
        ...
        for fqdn in result.result[ip]["fqdns"]:
            ...
            if "technologies" in result.result[ip]["fqdns"][fqdn]:
                for tech in result.result[ip]["fqdns"][fqdn]["technologies"]["tech"]:
                    if tech["name"] not in final[fqdn]["Technologies"]:
