...
def recursive_subdomains(subs: list, wt: int, wd: str, mode: str, domain: str) -> list:
    ...
    for subdomain in subs:
        logger.info(
            f"Recursive scan of {subdomain} | {subs.index(subdomain)}/{len(subs)}"
        )
        if "O" in mode:
    ...
    if len(new_subs) == 0:
        ...
        logger.info(
            f"Found {len(new_subs)} new subdomains, saved them to : exports/{domain}/dynamic_sub_save.txt"
        )
...
def menu():
    ...
    argpars.add_argument(
        ...
        required=False,
        default="OBS",
        help="Mode to use, O for OSINT (API request), B for bruteforce, S for IP scan (default OBS)",
    )
    argpars.add_argument(
        "-sF",
        "--subfile",
    ...
    argpars.add_argument(
        "-ipF", "--ipfile", required=False, help="Path to file with IPs, one per line"
    )
    argpars.add_argument(
        "-R",
        "--recursive",
    ...
    argpars.add_argument(
        ...
        type=str,
        required=False,
        help="Wordlist to use (small, medium(default), big)",
    )
    argpars.add_argument(
        "-wT",
        "--wordlistThreads",
        ...
        type=int,
        required=False,
        help="Number of threads to use for Wordlist(default 500)",
    )
    argpars.add_argument(
        "-iS",
        "--IPScanType",
        ...
        type=str,
        required=False,
        help="Choose what IPs to scan (W: only subdomains IP containing domain given, WR: only subdomains IP containtaining domain given but with a redirect, A: All subdomains detected",
    )
    argpars.add_argument(
        "-iT",
        "--IPthreads",
        ...
        type=int,
        required=False,
        help="Number of threads to use for IP scan(default 2000)",
    )
    argpars.add_argument(
        "-sT",
        "--subdomainsThreads",
        ...
        type=int,
        required=False,
        help="Number of threads to use for check real subdomains(default 500)",
    )
    argpars.add_argument(
        "-cP",
        "--checkPortsThreads",
        ...
        type=int,
        required=False,
        help="Check all ports of subdomains for all IP in IPScantype (-iS) and try to access them to check if it's a webport (default True) (deactivate with 0)",
    )
    argpars.add_argument(
        "-dT",
        "--detectTechno",
        ...
        type=bool,
        required=False,
        help="Detect techno used by subdomains (default True) (deactivate with False)",
    )
    argpars.add_argument(
        "-vuln",
        "--vulnScan",
        default=False,
        action="store_true",
        help="Scan subdomains using Nuclei, you need to have nuclei installed and in your PATH (default False)",
    )
    argpars.add_argument(
        "-vulnconf",
        "--vulnConfig",
        ...
        type=str,
        required=False,
        help="Path to config file for nuclei (default is the default config)",
    )
    argpars.add_argument(
        "-limit",
        "--limit",
        ...
        action="store_true",
        required=False,
        help="Limit the scope of scan to the domain given or the subdomains given in the file (-sF) (default False)",
    )
    ...
    if len(mode) > 3 or len(mode) < 1 or not re.match("^[OBS]+$", mode):
        logger.error(
            "Mode not valid, must be O B or S (or concatenation like : OS, OB, OBS)"
        )
        exit(1)
    if mode == "S":
        logger.error(
            "Cannot use only IP scan mode, you have to use at least one of the other modes with it"
        )
    ...
    if args.limit and args.subfile == None:
        logger.warning(
            "It's recommended to put a file with subdomains to scan when using limit mode"
        )
        input("Do you want to continue ? (y/n) ")
        if input == "n":
            exit(0)
    ...
    if args.domain:
        while not re.match(
            r"^[a-zA-Z0-9]+([\-\.]{1}[a-zA-Z0-9]+)*\.[a-zA-Z]{2,5}$", domain
        ):
        ...
        if "B" in mode:
            logger.info("Wordlist testing...")
            all_results += sh.from_wordlist_thread(
                domain, args.wordlistThreads, f"wordlists/{wordlist_size}.txt"
            )
            if args.limit and args.subfile != None:
                with open(args.subfile, "r") as file:
                    for line in file:
                        all_results += sh.from_wordlist_thread(
                            line.strip(),
                            args.wordlistThreads,
                            f"wordlists/{wordlist_size}.txt",
                        )
            logger.info("Wordlist testing done")
        ...
        logger.info(
            f"Actually creating the list of subdomains at : exports/{domain}/dynamic_sub_save.txt"
        )
        rp.dynamic_save(all_results, domain, "create")
        if args.recursive > 0:
            logger.info("Recursive scan...")
            new_scan = recursive_subdomains(
                all_results, wordlist_thread_number, wordlist_size, mode, domain
            )
            ...
            while len(new_scan) > 0 and counter < args.recursive:
                all_results += new_scan
                new_scan = recursive_subdomains(
                    new_scan, wordlist_thread_number, wordlist_size, mode, domain
                )
                all_results = rp.delete_occurences(all_results)
                counter += 1
        ...
        if "S" in mode or "B" in mode:
            ...
            (
                ...
                all_results, args.subdomainsThreads
            )
        ...
        final_dict = rp.result_filter(
            all_results, domain, subdomains_with_redirect, dead_subdomains, dns_exist
        )
        ...
        if "S" in mode:
            ...
            if args.IPScanType == "W":
                for ip in final_dict_result:
                    if (
                        final_dict_result[ip]["subdomains"]["subdomain_withdomain"]
                        != []
                    ):
                        open_ports = ips.port_scan_with_thread_limit(
                            ip, range(65536), args.IPthreads
                        )
                        for port in open_ports:
                            final_dict_result[ip]["ports"][port] = ips.detect_service(
                                ip, port
                            )
            elif args.IPScanType == "WR":
                for ip in final_dict_result:
                    if (
                        final_dict_result[ip]["subdomains"]["subdomain_with_redirect"]
                        != []
                        or final_dict_result[ip]["subdomains"]["subdomain_withdomain"]
                        != []
                    ):
                        open_ports = ips.port_scan_with_thread_limit(
                            ip, range(65536), args.IPthreads
            ...
            if args.checkPortsThreads != 0:
                final_dict_result = dp.detect_web_port(
                    final_dict_result, args.checkPortsThreads, args.IPScanType
                )
            if args.detectTechno:
                final_dict_result = dp.detect_web_techno(
                    final_dict_result, args.IPScanType
                )
                final_dict_result = dp.detect_web_techno_domain(
                    final_dict_result, args.IPScanType
                )
            if args.vulnScan:
                final_dict_result = ips.run_parse_nuclei(
                    final_dict_result, domain, args.IPScanType, args.vulnConfig
                )
        ...
        final_dict_result = {
            args.ip: {
                ...
                "techno": {"web": [], "web_domain": []},
                "vulns": [],
            }
        }
        logger.info("IP scanning...")
        open_ports = ips.port_scan_with_thread_limit(
            args.ip, range(65536), args.IPthreads
        )
        for port in open_ports:
            final_dict_result[args.ip]["ports"][port] = ips.detect_service(
                args.ip, port
            )
        logger.info("IP scanning done")
        logger.info("IP scanning service analysis...")
        ...
        if args.checkPortsThreads != 0:
            final_dict_result = dp.detect_web_port(
                final_dict_result, args.checkPortsThreads, "A"
            )
        if args.detectTechno:
        ...
        if args.vulnScan:
            ...
            final_dict_result = ips.run_parse_nuclei(
                final_dict_result, domain, "A", args.vulnConfig
            )
        logger.info("Detecting web ports done")
        logger.info("IP scanning results:")
        pprint(final_dict_result)
    ...
    with open(f"exports/{domain}/html_report_{domain}_{date}.html", "w") as file:
        file.write(html)
    logger.info(
        f"HTML report saved in exports/{domain}/html_report_{domain}_{date}.html"
    )
